...
def start_training(train_script, train_config, sample_prompts):
    ...
    command = f"bash {resolve_path('train.sh')}" if sys.platform != "win32" else resolve_path_without_quotes('train.bat')
    env = os.environ.copy()
...
""", elem_classes="group_padding")
    ...
    num_repeats.change(update_total_steps, inputs=[max_train_epochs, num_repeats, images], outputs=[total_steps])
    images.upload(update_total_steps, inputs=[max_train_epochs, num_repeats, images], outputs=[total_steps])
    images.delete(update_total_steps, inputs=[max_train_epochs, num_repeats, images], outputs=[total_steps])
    images.clear(update_total_steps, inputs=[max_train_epochs, num_repeats, images], outputs=[total_steps])
    ...
    start.click(fn=create_dataset, inputs=[dataset_folder, resolution, images] + caption_list, outputs=dataset_folder).then(
        fn=start_training, inputs=[train_script, train_config, sample_prompts], outputs=terminal
    )
